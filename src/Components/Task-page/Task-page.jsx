...
import {
  ...
  AddNewTask,
  FetchProjectMembers,
} from "../../scripts/API/access-projectsTasks";
...
export default function TaskPage() {
  ...
  async function SetMembersData() {
    ...
    } else console.error("Error fetching projects members");
  }
  ...
  function OnClickTaskCard(aTaskId) {
    ...
    } else console.error("Missing projectMembers!!");
  }
  ...
  return (
    <MemberContext.Provider
      value={{ projectMembers, RefetchProjectData: InitializePage }}
    >
      {currentUser && bTaskDisplayOn && oTaskToDisplay && (
        <TaskDisplay
          project_id={project_id}
          taskID={oTaskToDisplay.id}
          handleCloseDisplay={OnCloseDisplay}
          assignedMemberID={oTaskToDisplay.assignedMemberID}
          title={oTaskToDisplay.title}
          description={oTaskToDisplay.description}
          priority={oTaskToDisplay.priority}
          userRole={currentUser.role}
          bCreating={!oTaskToDisplay.id}
          progress={oTaskToDisplay.progress}
        />
      )}
      <div className="task-page">
                  ...
                  if (task.progress == progress.pending)
                    return (
                      <TaskCard
                        ...
                        onClick={() => OnClickTaskCard(task.id)}
                      ></TaskCard>
                    );
                })}
              </div>
        ...
        {currentUser && currentUser.role > 1 && bTaskDisplayOn === false && (
          <button className="button_newTask" onClick={CreateNewTasks}>
            <img src={icon_addTask} alt="" />
          </button>
        )}{" "}
      </div>
    </MemberContext.Provider>
